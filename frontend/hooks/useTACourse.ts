...
export function useTACourse() {
  ...
  const CourseRoles = useCallback(
    async (courseId: number, forced: boolean = false) => {
  ...
  const AddMarks = useCallback(
    async (courseId: number, assessment_id: number, marksData: AddMarksRequest) => {
      if (!user?.id) {
        setError('User not found');
        return;
      }
  ...
  const EnrollStudent = useCallback(
    async (courseId: number, EnrollStudentData: EnrollStudentRequest) => {
      if (!user?.id) {
        setError('User not found');
        return;
      }
  ...
  const UnEnrollStudent = useCallback(
    async (courseId: number, student_id: number) => {
      if (!user?.id) {
        setError('User not found');
        return;
      }
  ...
  const GetMarksOfAssessment = useCallback(
    async (courseId: number, assessment_id: number) => {
  ...
  const DeleteStudentMarks = useCallback(
    async (courseId: number, assessment_id: number, student_id: number) => {
      if (!user?.id) {
        setError('User not found');
        return;
      }
  ...
  const UnpublishMarks = useCallback(
    async (courseId: number, assessment_id: number) => {
      if (!user?.id) {
        setError('User not found');
        return;
  ...
  const GetAllAssessments = useCallback(
    async (courseId: number, forced: boolean = false) => {
      ...
      try {
        ...
        const assessmentsList = Array.isArray(response)
          ? response
  ...
  const GetAllPolicy = useCallback(
  ...
  const GetPolicyById = useCallback(
    async (courseId: number, policy_id: number) => {
      if (!user?.id) {
        setError('User not found');
        return;
      }
  ...
  const GetTotalScores = useCallback(
    async (courseId: number) => {
  ...
  const SignUpUser = useCallback(
    async (courseId: number, signUpData: SignUpRequest) => {
      if (!user?.id) {
        setError('User not found');
        return;
      }
