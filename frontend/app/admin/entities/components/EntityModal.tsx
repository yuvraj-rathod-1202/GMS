              ...
              <div className="max-h-40 overflow-y-auto border border-gray-100 rounded-xl divide-y divide-gray-50 scrollbar-hide">
                ...
                {courses.map((course) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, course_id: course.id, course_name: course.name })
                    }
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className={`p-1.5 rounded ${formData.course_id === course.id ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400'}`}
                      >
            ...
            {formData.course_id && (
              <div className="text-[11px] text-blue-600 bg-blue-50 p-2 rounded-lg border border-blue-100">
                ...
                <span className="font-bold">
                  {formData.course_name || `ID: ${formData.course_id}`}
                </span>
              </div>
            )}
            <div className="space-y-1">
          ...
          <>
              ...
              <div className="max-h-40 overflow-y-auto border border-gray-100 rounded-xl divide-y divide-gray-50 scrollbar-hide">
                ...
                {users.map((user) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, student_id: user.id, email: user.email })
                    }
                    className={`w-full flex items-center justify-between p-2 text-left transition-colors hover:bg-gray-50 ${
            ...
            {(formData.user_id || formData.student_id) && (
              <div className="text-[11px] text-blue-600 bg-blue-50 p-2 rounded-lg border border-blue-100">
                Selected:{' '}
                <span className="font-bold">
                  {formData.email || formData.user_id || formData.student_id}
                </span>
              </div>
            )}
            <div className="space-y-1">
          ...
          <div className="space-y-4">
            ...
            <div className="max-h-60 overflow-y-auto border border-gray-100 rounded-xl divide-y divide-gray-50 scrollbar-hide">
              ...
              {users.map((user) => (
                ...
                >
                  ...
                  {formData.id === user.id && <div className="w-2 h-2 rounded-full bg-blue-600" />}
                </button>
              ))}
            ...
            {!isEdit && (
              <p className="text-[10px] text-gray-500 italic">
                Promoting a user to Admin grants them full system access. Please verify the user
                carefully.
              </p>
            )}
            ...
            {formData.id && (
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-100 flex items-center justify-between">
                <div className="text-xs text-blue-700">
                  Selected:{' '}
                  <span className="font-bold">
                    {users.find((u) => u.id === formData.id)?.email || formData.id}
                  </span>
                </div>
                <button
                  type="button"
