...
export const useCourseDetailStore = create<CourseDetailState>((set) => ({
  ...
  setHasFetchedTADataInSession: (key, value) =>
    set((state) => ({
      hasFetchedTADataInSession: { ...state.hasFetchedTADataInSession, [key]: value },
    })),
  setHasFetchedInstructorDataInSession: (value) =>
    set({ hasFetchedInstructorDataInSession: value }),
  clearCourseDetail: () =>
    set({
      currentCourse: null,
      currentAssessment: null,
