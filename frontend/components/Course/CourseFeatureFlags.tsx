...
export const CourseFeatureFlags = ({ courseId }: { courseId: string }) => {
  ...
  const handleToggle = async (flag: CourseFlag) => {
    const currentEnabled =
      flag.override_enabled !== null ? flag.override_enabled : flag.default_enabled;
    ...
    try {
      ...
      setFlags(
        flags.map((f) => (f.name === flag.name ? { ...f, override_enabled: nextEnabled } : f))
      );
    } catch (err) {
      if (process.env.NEXT_PUBLIC_ENVIRONMENT === 'development') {
      ...
      <div className="flex items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Enable or disable specific functionalities for this course.
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-all ${isEnabled ? 'translate-x-6' : 'translate-x-1'} ${saving === flag.name ? 'scale-75 animate-pulse' : ''}`}
                  />
                </button>
                <span
                  className={`text-[10px] font-black uppercase tracking-tighter ${isEnabled ? 'text-gms-blue' : 'text-gray-400'}`}
                >
