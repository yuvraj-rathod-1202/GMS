...
export default function calculateTotalMarks(student_id: number, policy: PolicyDBObject, assessments: AssessmentDBObject[], assessmentMarks: Record<number, MarksDBObject[]>) {
    let total_score = 0.0;
    ...
    for (let component of policy.components) {
        ...
        const marks_in_category = assessments.filter(a => a.assessment_type_id === component.assessment_category_id).map(a => {
        ...
        if (component_rule_type === 'CUMULATIVE') {
            ...
            total_score +=  (component_max_marks > 0) ? (component_marks_obtained * component_total_weightage) / component_max_marks : 0;
        } else if (component_rule_type === 'EQUAL_WEIGHTAGE') {
            for (let marks of marks_in_category) {
                component_score += marks[1].marks_obtained*100/marks[0].max_marks;
            }
            total_score += (component_score > 0) ? (component_score * component_total_weightage) / (100*marks_in_category.length) : 0;
        } else if (component_rule_type === 'BEST_N') {
            const n = component_rule_params?.n || 0;
            const sorted_marks = marks_in_category.sort((a, b) => b[1].marks_obtained*100/ b[0].max_marks - a[1].marks_obtained*100/a[0].max_marks);
            const best_n_marks = sorted_marks.slice(0, n);
            for (let marks of best_n_marks) {
                component_score += marks[1].marks_obtained*100/marks[0].max_marks;
            }
            total_score += (component_score > 0) ? (component_score * component_total_weightage) / (100*best_n_marks.length) : 0;
        } else if (component_rule_type === 'CUSTOM') {
            const logic = component_rule_params;
            for (let marks of marks_in_category) {
                component_score += (marks[1].marks_obtained*100/marks[0].max_marks) * (logic?.[marks[0].id] || 0) / 100;
            }
...
export function calculateTotalMarksOptimized(
    ...
    for (let component of policy.components) {
        ...
        if (component_rule_type === 'CUMULATIVE') {
            ...
            total_score += (component_max_marks > 0) 
            ...
            const logic = component_rule_params;
            for (let [assessment, marks] of marks_in_category) {
                component_score += ((marks * 100) / assessment.max_marks) * (logic?.[assessment.id] || 0) / 100;
            }
            total_score += component_score;
        }
    }
