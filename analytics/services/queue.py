...
def update_course_analytics_in_db(course_id: int, new_entries: list, old_entries: list, deleted_entries: list, course_analytic_overview):
    ...
    try:
        ...
        for new in new_entries:
            freq_changes[new] = freq_changes.get(new, 0) + 1
        for deleted_mark in deleted_entries:
            freq_changes[deleted_mark] = freq_changes.get(deleted_mark, 0) - 1
        for old_mark, new_mark in old_entries:
            freq_changes[old_mark] = freq_changes.get(old_mark, 0) - 1
            freq_changes[new_mark] = freq_changes.get(new_mark, 0) + 1
        
        freq_update = [(course_id, mark, change) for mark, change in freq_changes.items()]
...
def update_assessment_analytics_in_db(course_id: int, assessment_id: int | None, new_entries: list, old_entries: list, deleted_entries: list, assessment_analytic_overview):
    db = get_db()
    if not db:
        raise Exception("Database connection error")
    ...
    try:
        ...
        freq_update = [(course_id, assessment_id, mark, change) for mark, change in freq_changes.items()]
        ...
        cursor.execute(
            ...
            WHERE assessment_id = %s
            """,
            (new_total_students, new_mean, new_std, new_median, new_max, new_min, assessment_id)
        )
...
def update_assessment_analytics(data: AssessmentQueueMessage):
    ...
    if not assessment_analytics_overview:
        ...
        update_assessment_analytics_in_db(data.course_id, data.assessment_id, new_entry, old_entry, deleted_entry, assessment_analytics_overview)
