#!/bin/bash
set -e

print_blue()   { echo -e "\e[44;97m $1 \e[0m"; }
print_yellow() { echo -e "\e[43;30m $1 \e[0m"; }

print_blue "Starting deployment process..."

print_yellow "Auth Service Deployment"

cd auth_user

docker build -t yrrathod/gms_auth:latest .
docker push yrrathod/gms_auth:latest
kubectl rollout restart deployment auth

print_yellow "Courses Service Deployment"

cd ../courses

docker build -t yrrathod/gms_courses:latest .
docker push yrrathod/gms_courses:latest
kubectl rollout restart deployment courses

print_yellow "Gateway Service Deployment"

cd ../gateway

docker build -t yrrathod/gms_gateway:latest .
docker push yrrathod/gms_gateway:latest
kubectl rollout restart deployment gateway

print_yellow "Marks Service Deployment"

cd ../marks

docker build -t yrrathod/gms_marks:latest .
docker push yrrathod/gms_marks:latest
kubectl rollout restart deployment marks

print_yellow "Analytics Service Deployment"

cd ../analytics

docker build -t yrrathod/gms_analytics:latest .
docker push yrrathod/gms_analytics:latest
kubectl rollout restart deployment analytics

print_yellow "Policy Service Deployment"

cd ../policy

docker build -t yrrathod/gms_policy:latest .
docker push yrrathod/gms_policy:latest
kubectl rollout restart deployment policy

print_yellow "Frontend Deployment"

cd ../frontend

docker build -t yrrathod/gms_frontend:latest .
docker push yrrathod/gms_frontend:latest
kubectl rollout restart deployment frontend

print_yellow "Deployment process completed."